...
function InstanceSelect(props){    
        return (
            <Row>
                ...
                <Col lg='auto' md='auto' sm='auto'>                                           
                        <Button  size='sm'  variant='primary' disabled={!props.selected_row.OOZIE_JOB_URL} onClick={()=>window.open((props.selected_row.OOZIE_JOB_URL.substring(props.selected_row.OOZIE_JOB_URL.indexOf('http'),props.selected_row.OOZIE_JOB_URL.length)))}>
                            <FaExternalLinkSquareAlt className='mb-1'/>
                            <span style={{whiteSpace : 'pre'}}>  Visit Oozie URL</span>                    
                        </Button>                                    
                </Col>                
                <Col lg='auto' md='auto' sm='auto'>
                    <Button  size='sm' variant='primary' disabled={true} style={{opacity:0}}>
